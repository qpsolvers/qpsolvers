...
class TestSolveQP(unittest.TestCase):
    ...
    def get_test_no_ineq(solver: str):
        ...
        def test(self):
            ...
            eq_tolerance = (
                1e-5
                if solver == "jaxopt_osqp"
                else 1e-6 if solver in ["qpax", "sip"] else 1e-9
            )
            self.assertLess(
                norm(x - known_solution), sol_tolerance, f"{solver=}"
            )
    ...
    def get_test_warmstart(solver: str):
        ...
        def test(self):
            ...
            eq_tolerance = 1e-4 if solver in ["jaxopt_osqp", "osqp", "sip"] else 1e-10
            ineq_tolerance = (
                1e-3
                if solver in ["osqp", "qpalm", "scs"]
                else (
                ...
                if solver in ["jaxopt_osqp", "osqp"]
                    ...
                    if solver in ["qpalm", "scs", "qpax"]
                        ...
                        if solver == "proxqp"
                            ...
                            1e-6
                            if solver in ["cvxopt", "kvxopt", "mosek", "qpswift", "piqp", "sip"]
                            else 1e-8
                        )
                    )
                )
